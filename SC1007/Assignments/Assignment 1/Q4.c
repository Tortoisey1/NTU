...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    if (size % 2 == 1)
    {
        // front list is longer
        int newFrontSize = (size / 2) + 1;
        int newBackSize = (size / 2);
        int backIndex = newFrontSize;

        // insert into front list
        for (int i = 0; i < newFrontSize; i++)
        {
            curr = findNode(ll, i);
            insertNode(resultFrontList, i, curr->item);
        }

        // insert into back list
        for (int j = 0; j < newBackSize; j++)
        {
            curr = findNode(ll, (backIndex + j));
    ...
    else
    {
        ...
        // start from halfway from original list
