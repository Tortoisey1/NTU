...
void interleaveQueue(Queue *q)
{
    ...
    int half = (q->ll.size) / 2, temp, fullSize = q->ll.size;
    ...
    // alternate add back to queue

    for (int i = 0; i < half; i++)
    {

        // add back from stack to queue
        enqueue(q, pop(tempStack));

        // add front of queue to back of queue
