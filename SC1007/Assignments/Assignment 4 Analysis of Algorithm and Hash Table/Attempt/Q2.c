...
{
    // find the hash to get to the linked list at the correct array index
    int hashValue = Hash(key, Q1Ptr->hSize);

    // get the linked list
    HashTableNode *temp = &(Q1Ptr->Table[hashValue]);
    ListNode *current = temp->head;
    ...
    // insert into the linked list - just put at the start of linked list
...
{ // check if  key doesn't exist
    ...
    else
    {
        ...
        // get the linked list
        HashTableNode *temp = &(Q1Ptr->Table[hash]);
        ListNode *current = temp->head;

        // if first node of the linked list
        if (current->key == key)
        {
            if (current->pre == NULL)
            {
