...
int SD(Graph g, int v, int z)
{
    ...
    // empty graph or same vertex or vertex dont exist
    if (g.V == 0 || v == z || v <= 0 || z <= 0 || v > g.V || z > g.V)
    {
        return -1;
    }
    ...
    while (!isEmptyQueue(*q))
    {
        ...
        {
            // edge present
            if (g.matrix[current - 1][i] == 1)
            {
                // check if visited - not visited
                if (g.visited[i] == 0)
                {
                    // if is the target edge
                    if (i == z - 1)
                    {
                        return g.visited[current - 1];
                    }

                    // add the vistied number as the length
                    g.visited[i] = g.visited[current - 1] + 1;
